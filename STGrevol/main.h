...
Exitmode_select exitmode_select(&define_srsize.enlargement_factor, &loadgr.exitgr, TITLEMEN_BASIC_PLACE_X, TITLEMEN_BASIC_PLACE_Y, 0, 0, 0.0, &titlemenue_system.select_menue_var, EXIT_MENUE_NUMBER, &joypad_control.shot_flag, &joypad_control.prev_chattering_flag);
...
Keyconmode_select keyconmode_select(&joypad_control.downkey_flag, &system_control.system_var, &define_srsize.enlargement_factor, &loadgr.keycongr, TITLEMEN_BASIC_PLACE_X, TITLEMEN_BASIC_PLACE_Y, 0, 0, 0.0, &titlemenue_system.select_menue_var, KEYCON_MENUE_NUMBER, &loadgr.keycon_num, &loadgr.exitgr, &loadgr.keycon_backgr, &joypad_control.shot_flag, &joypad_control.shot_keep_flag, &joypad_control.upkey_flag, &joypad_control.prev_chattering_flag);